#!/bin/bash
# 对每个alpha值额外跑2次（run2/run3），已有run1
set -e
REPO="$(dirname "$0")/.."
REPO="$(cd "$REPO" && pwd)"

ALPHAS="0.1 0.3 0.5 0.7 1.0 1.5"
OUT_DIR="$REPO/evaluation/stats/alpha-sweep"
mkdir -p "$OUT_DIR"

for RUN in 2 3; do
    for ALPHA in $ALPHAS; do
        OUTFILE="$OUT_DIR/alpha${ALPHA}_run${RUN}.json"
        if [ -f "$OUTFILE" ]; then
            echo "[SKIP] $OUTFILE already exists"
            continue
        fi
        echo "========================================"
        echo "[START] alpha=$ALPHA run=$RUN  $(date '+%Y-%m-%d %H:%M:%S')"
        echo "========================================"
        cd "$REPO/zip-diff"
        ./target/release/fuzz \
            -b 50 \
            --batch-timeout-secs 300 \
            -s 3600 \
            --parsers-dir "$REPO/parsers" \
            --input-dir "$REPO/evaluation/input" \
            --output-dir "$REPO/evaluation/output" \
            --samples-dir "$REPO/evaluation/samples" \
            --results-dir "$REPO/evaluation/results" \
            --coverage-ucb-alpha "$ALPHA" \
            --stats-file "$OUTFILE"
        echo "[DONE]  alpha=$ALPHA run=$RUN  $(date '+%Y-%m-%d %H:%M:%S')"
    done
done

echo "All multi-run alpha-sweep complete."
