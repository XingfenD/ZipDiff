...
# Script to build and run podman services with custom entrypoint
...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  -t, --target TARGET    Specify the build target for podman build"
    echo ""
...
# Build the image using podman directly
if podman build -f "$SERVICE_NAME/Dockerfile" -t "localhost/parsers_${SERVICE_NAME}:$IMAGE_TAG" $BUILD_OPTIONS "$PARSERS_DIR"/"$SERVICE_NAME"; then
...
RUN_CMD="podman run --rm -it --entrypoint /bin/sh"
SHOW_CMD="podman run --rm"
